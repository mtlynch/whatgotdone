...
func (s *defaultServer) routes() {
	...
	api.HandleFunc("/entry/{date}", s.requireAuthentication(s.entryDelete())).Methods(http.MethodDelete)
	...
	api.HandleFunc("/draft/{date}", s.requireAuthentication(s.draftDelete())).Methods(http.MethodDelete)
