...
func (s *defaultServer) routes() {
	...
	s.router.HandleFunc("/api/user/{username}", s.enableCors(s.userOptions())).Methods(http.MethodOptions)
	s.router.HandleFunc("/api/user/{username}", s.enableCors(s.userGet())).Methods(http.MethodGet)
	s.router.HandleFunc("/api/user", s.enableCors(s.userPost())).Methods(http.MethodPost)
