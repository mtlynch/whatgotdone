...
func (s *defaultServer) routes() {
	...
	s.router.PathPrefix("/android-chrome-192x192.png").Handler(fs)
	s.router.PathPrefix("/android-chrome-512x512.png").Handler(fs)
	s.router.PathPrefix("/app.js").Handler(fs)
	s.router.PathPrefix("/apple-touch-icon.png").Handler(fs)
	s.router.PathPrefix("/browserconfig.xml").Handler(fs)
	s.router.PathPrefix("/favicon-16x16.png").Handler(fs)
	s.router.PathPrefix("/favicon-32x32.png").Handler(fs)
	s.router.PathPrefix("/favicon.ico").Handler(fs)
	s.router.PathPrefix("/mstile-150x150.png").Handler(fs)
	s.router.PathPrefix("/site.webmanifest").Handler(fs)
